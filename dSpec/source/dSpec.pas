...
type
  ...
  private
    FContext: IContext;
  public
    constructor Create(AContext : IContext);
  ...
  private
    ...
    procedure SetAutoDoc(Value : IAutoDoc);
...
constructor TSpecify.Create(AContext: IContext);
begin
  inherited Create;
  FContext := AContext;
end;
...
begin
  Result := TIntegerSpecifier.Create(Value, CallerAddr, FContext, ValueName);
  FContext.NewSpecDox(GetValueName(Value, ValueName));
end;

function TSpecify.That(Value: Boolean; const ValueName : string = ''): ISpecifier;
begin
  Result := TBooleanSpecifier.Create(Value, CallerAddr, FContext, ValueName);
  FContext.NewSpecDox(GetValueName(Value, ValueName));
end;

function TSpecify.That(Value: Extended; const ValueName : string = ''): ISpecifier;
begin
  Result := TFloatSpecifier.Create(Value, CallerAddr, FContext, ValueName);
  FContext.NewSpecDox(GetValueName(Value, ValueName));
end;

function TSpecify.That(Value: string; const ValueName : string = ''): ISpecifier;
begin
  Result := TStringSpecifier.Create(Value, CallerAddr, FContext, ValueName);
  FContext.NewSpecDox(GetValueName(Value, ValueName));
end;

function TSpecify.That(Value: TObject; const ValueName : string = ''): ISpecifier;
begin
  Result := TObjectSpecifier.Create(Value, CallerAddr, FContext, ValueName);
  if ValueName <> '' then
    FContext.NewSpecDox(ValueName)
  else
  if Assigned(Value) then
    FContext.NewSpecDox(Value.ClassName)
  else
    FContext.NewSpecDox('TObject');
end;

function TSpecify.That(Value: Pointer; const ValueName : string = ''): ISpecifier;
begin
  Result := TPointerSpecifier.Create(Value, CallerAddr, FContext, ValueName);
  if ValueName <> '' then
    FContext.NewSpecDox(ValueName)
  else
    FContext.NewSpecDox('Pointer');
end;

function TSpecify.That(Value: TClass; const ValueName : string = ''): ISpecifier;
begin
  Result := TClassSpecifier.Create(Value, CallerAddr, FContext, ValueName);
  if ValueName <> '' then
    FContext.NewSpecDox(ValueName)
  else
    FContext.NewSpecDox(Value.ClassName);
...
begin
  Result := TDateTimeSpecifier.Create(Value, CallerAddr, FContext, ValueName);
  FContext.NewSpecDox(GetValueName(Value, ValueName));
end;

function TSpecify.That(Value: IInterface; const ValueName : string = ''): ISpecifier;
begin
  Result := TInterfaceSpecifier.Create(Value, CallerAddr, FContext, ValueName);
  FContext.NewSpecDox(GetValueName(Value, ValueName));
end;
...
procedure TContext.SetAutoDoc(Value: IAutoDoc);
begin
  FAutoDoc := Value;
